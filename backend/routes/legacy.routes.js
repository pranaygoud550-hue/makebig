...
/** Set by registerLegacyRoutes — used by pushNotification for Socket.io emits. */
let legacyIo = null;
...
async function pushNotification({
  ...
  try {
    ...
    legacyIo?.to(`user_${uid}`).emit("notification_received", payload);
    if (contactNorm) {
      legacyIo?.to(`contact_${contactNorm}`).emit("notification_received", payload);
    }
...
export { pushNotification, getTeammateContacts };
