...
export function useProjectSocket(projectId, userId, userName, userContact, token) {
  ...
  const handleIncomingMessage = useCallback((message) => {
    if (!message) return;
    console.log('[socket] new_message received', messageId(message), message);
  ...
  useEffect(() => {
    ...
    async function connect() {
      ...
      if (!authToken || cancelled) {
        console.log('[socket] no auth token — skipping connect');
        return;
      }

      const apiUrl = getApiOrigin();
      console.log('[socket] connecting to', apiUrl, 'project', projectId);
      ...
      const joinRoom = () => {
        console.log('[socket] join_project → room project_' + projectId);
      ...
      socket.on('user_joined', (data) => {
        console.log('[socket] user_joined', data.userName);
      });

      socket.on('user_left', (data) => {
        console.log('[socket] user_left', data.userName);
      });
      ...
      socket.on('project_changed', (data) => {
        console.log('[socket] project_changed', data);
      });
      ...
      socket.on('notification_received', (notification) => {
        console.log('[socket] notification_received', notification);
      });
  ...
  const sendMessage = async (content, type = 'text') => {
    ...
    if (socketRef.current?.connected && !isSupabaseConfigured) {
      console.log('[socket] send_message via socket');
    ...
    try {
      console.log('[socket] send_message via REST fallback');
