...
import { isCollegeEmail } from './collegeEmail.ts';
...
function toClientUser(user) {
  ...
  return {
    ...
    skillTestStatus: user.skillTestStatus || (user.verifiedSkills?.length ? 'completed' : 'pending'),
...
export async function upsertVerifiedUser(body, { requireVerified = true } = {}) {
  ...
  const pendingIds = Array.isArray(pendingSkillIds)
    ? pendingSkillIds.map((id) => clampString(String(id), 64)).filter(Boolean)
    : [];
  const skillTestStatus =
    verifiedList.length > 0 ? 'completed' : skillTestSkipped || pendingIds.length ? 'pending' : 'pending';

  const collegeEmailVerified = isCollegeEmail(normalized);
  ...
  const updatePayload = {
    ...
    skillTestStatus,
    ...(verifiedList.length ? { verifiedSkills: verifiedList, pendingSkillIds: [] } : {}),
    ...(!verifiedList.length && pendingIds.length ? { pendingSkillIds: pendingIds } : {}),
    ...(!verifiedList.length && skillTestSkipped && !pendingIds.length ? { pendingSkillIds: [] } : {}),
